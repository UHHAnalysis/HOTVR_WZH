...
RocHistsComb::RocHistsComb(const char* name) : BaseHists(name)
{
  // named default constructor
   
}

RocHistsComb::~RocHistsComb()
{
  // default destructor, does nothing
}

void RocHistsComb::Init()
{
  ...
  Book( TH1F( "pT_s1_all"," p_{T} topjets (selection)",50,0,2000));
  ...
  Book( TH1F( "pT_s1_cms_nsub_tagged"," p_{T}  nsubjettiness (cms tagged)",50,0,2000));
  Book( TH1F( "pT_s1_tobi_tagged"," p_{T} topjets (tobi tagged)",50,0,2000));
  Book( TH1F( "pT_s1_tobi2_tagged"," p_{T} topjets (tobi tagged)",50,0,2000));
  Book( TH1F( "pT_s1_tobi3_tagged"," p_{T} topjets (tobi tagged)",50,0,2000));
  Book( TH1F( "pT_s1_tobi4_tagged"," p_{T} topjets sd mass (tobi tagged)",50,0,2000));
  Book( TH1F( "pT_s1_tobi5_tagged"," p_{T} topjets nsubjettiness (tobi tagged)",50,0,2000));
  Book( TH1F( "pT_s1_tobi6_tagged"," p_{T} topjets w mass (tobi tagged)",50,0,2000));
...
//   Book( TH3D( "mistag_roc_tagged_cvsm", ";w;nsub;Pt parton (GeV)",nsteps,0.0,0.5,nsteps,0.5,1.0,50,0.0,10000.0)); 
 ...
 Book(TH1F("Jetm00_800","jet multiplicity 01",50,0,10));
  ...
  Book( TH1D( "mmin2", ";Top jet mass (GeV);Events", 50, 0, 200));
   ...
   Book(TH1D("mjet500","mjet",100,0,300));
...
void RocHistsComb::SetIdVersion(TString s)
{
  idVersion=s;
}

void RocHistsComb::Fill()
{
  // fill the histograms
  ...
  double min_r(0.1), max_r(1.5),mw(40.);//double mu(30.), theta(0.7);
...
//EFFICIENCY
  ...
  if(calc->GetEventNum()==4252236){
    for(int o=0;o<SortedJets.size();o++) std::cout<<"Fatjets "<<SortedJets[o].pt()<<" "<<SortedJets[o].phi()<<std::endl;
  }
  /* std::vector<fastjet::PseudoJet> constituents;
  for(int o;o<SortedSubJets.size();o++) {
 ...
 //SortedJets = sorted_by_pt(inclusive_jets_varR) ;
  ...
  Hist("NJetstop")->Fill(bcc->topjets->size(),weight);
	...
	if(bestjetindex2>-1 )
	  {
	    std::vector<fastjet::PseudoJet> SortedSubJets2;
	    std::vector<fastjet::PseudoJet> jetpart2 = varjets[bestjetindex2].constituents();
	    double ptmin(10.);
	    // std::cout<<rho<<std::endl;
	    vector<fastjet::PseudoJet> inclusive_jetsCA2;
	    HHTopTagger pluginAKT2(mu, theta,min_r, max_r,6000000,mw, HHTopTagger::AKTLIKE,fatjets);
	    fastjet::JetDefinition jet_defCA3(&pluginAKT2);
	     
	    fastjet::ClusterSequence clust_seqCA2(jetpart2, jet_defCA3);
	    inclusive_jetsCA2 = clust_seqCA2.inclusive_jets(ptmin);
  //vector<fastjet::PseudoJet> inclusive_jetsCA = clust_seqCA.inclusive_jets(ptmin);
	    fatjets=pluginAKT2.get_fatjets();
	    for(int o=0;o<fatjets.size();o++) if(fatjets[o].user_index()!=-1) SortedSubJets2.push_back(fatjets[o]);
	    //SortedSubJets2 = sorted_by_pt(inclusive_jetsCA2);
	    if(calc->GetEventNum()==4252236){
	     
		std::cout<<"Fatjets "<<SortedJets[bestjetindex2].pt()<<" "<<SortedJets[bestjetindex2].phi()<<std::endl;
		for(int t=0;t<SortedSubJets2.size();t++) std::cout<<"Subjets "<<SortedSubJets2[t].pt()<<" "<<SortedSubJets2[t].phi()<<std::endl;
	      
	    }
	    
	    std::cout<<"CIRCLE2"<<std::endl;
	      Hist("nsubjets_massjump")->Fill(SortedSubJets2.size(),weight);
	  ...
	  if(SortedSubJets2.size()>0){ fastjet::PseudoJet sum=SortedSubJets2[0];
	  for(int mj=1;mj<SortedSubJets2.size();mj++) {
	    sum.operator+=(SortedSubJets2[mj]);
	  }
	  ...
	  */
	  if(SortedSubJets2.size()>2 && SortedJets[bestjetindex2].m()>150 &&  SortedJets[bestjetindex2].m()<250 && mmin2>50)  Hist("pT_s1_tobi3_tagged")->Fill(Had_Tops[j].pt(),weight);
	  ...
	  double distance=100.;
	   ...
	   Hist("mjet")->Fill(mjet,weight);
	   if(Had_Tops[j].pt()>200)  Hist("mjet200")->Fill(mjet,weight);
	   if(Had_Tops[j].pt()>500)  Hist("mjet500")->Fill(mjet,weight);
	   if(Had_Tops[j].pt()>600)  Hist("mjet600")->Fill(mjet,weight);
	   if(Had_Tops[j].pt()>800)  Hist("mjet800")->Fill(mjet,weight);
	   if(Had_Tops[j].pt()>1000)  Hist("mjet1000")->Fill(mjet,weight);
  ...
  for(int o=0;o<fatjets.size();o++) if(fatjets[o].user_index()==-1) Hist("pT_jet1")->Fill(fatjets[o].perp(),weight);*/
   ...
   // std::cout<<"*****"<<std::endl;
    ...
    /*td::vector<int> jetindex[fatjets.size()];
  ...
  varjets=SortedJets;
   ...
   //for(int o=0;o<decay_products.size();o++) ((TH2D*)Hist("JetDisplay_decay"))->Fill(decay_products[o].phi(),decay_products[o].eta(),decay_products[o].pt());
     ...
     if (bestjetindex>-1)
     {
       ...
       // std::cout<<rho<<std::endl;
       vector<fastjet::PseudoJet> inclusive_jetsCA2;
       // fastjet::JetDefinition jet_defCA3;
       ClusteringVetoPlugin pluginAKT2(mu, theta, max_r, ClusteringVetoPlugin::AKTLIKE);
       fastjet::JetDefinition jet_defCA3(&pluginAKT2);
       ...
       SortedSubJets = sorted_by_pt(inclusive_jetsCA2);
	 ...
	 sum.operator+=(SortedSubJets[mj]);
	  ...
	  if(SortedSubJets.size()>2 && varjets[i].m()>150 &&  varjets[i].m()<250 && mmin2>50)  Hist("pT_s1_tobi3_tagged")->Fill(genparticles->at(bestjetindex).pt(),weight);
	  if(SortedSubJets.size()>2 && varjets[i].m()>150 &&  varjets[i].m()<250 && wmass_rec>70 &&wmass_rec<90)  Hist("pT_s1_tobi6_tagged")->Fill(genparticles->at(bestjetindex).pt(),weight);
       ...
       Hist("nsubjets")->Fill(nsubjets,weight);
       mjet=SortedJets[i].m();
       if(genparticles->at(bestjetindex).pt()>200)  Hist("mjet200")->Fill(mjet,weight);
       if(genparticles->at(bestjetindex).pt()>500)  Hist("mjet500")->Fill(mjet,weight);
       if(genparticles->at(bestjetindex).pt()>600)  Hist("mjet600")->Fill(mjet,weight);
       if(genparticles->at(bestjetindex).pt()>800)  Hist("mjet800")->Fill(mjet,weight);
       if(genparticles->at(bestjetindex).pt()>1000)  Hist("mjet1000")->Fill(mjet,weight);
