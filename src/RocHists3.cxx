...
RocHists3::RocHists3(const char* name) : BaseHists(name)
{
...
void RocHists3::Init()
{
  ...
  Book( TH1F("pT_s1_gluons","pT gluons",75,0,3000));
 Book( TH1F("pT_s1_gluons_all","pT gluons",75,0,3000));
 Book( TH1F("pT_s1_quarks","pT gluons",75,0,3000));
  Book( TH1F("pT_s1_quarks_all","pT gluons",75,0,3000));
...
//   Book( TH3D( "mistag_roc_tagged_cvsm", ";w;nsub;Pt parton (GeV)",nsteps,0.0,0.5,nsteps,0.5,1.0,50,0.0,10000.0)); 
 ...
 Book(TH1F("Jetm00_800","jet multiplicity 01",50,0,10));
  ...
  Book( TH1D( "mmin2", ";Top jet mass (GeV);Events", 50, 0, 200));
   ...
   Book(TH1D("tau3tau2_400_massjump","tau3tau2 pt>500GeV massjump",100,0,1.0));
 ...
 Book( TH1D("nsub_eff200","eff nsub",100,-0.005,0.995));
  ...
  Book(TH1D("delta_top_jet","delta betwen top jet",120,0,1));
    ...
    Book(TH1D("delta_top_jet600","delta betwen top jet",100,0,1));
     Book(TH1D("delta_top_jet800","delta betwen top jet",100,0,1));
      Book(TH1D("delta_top_jet1000","delta betwen top jet",100,0,1));
       ...
       Book(TH1D("pdg_all","pdg id",61,-30.5,30.5));
        Book(TH1D("pdg_tagged","pdg id",61,-30.5,30.5));
 Book(TH1D("pdg_highestpt","pdg id",61,-30.5,30.5));
 Book(TH1D("parton_highest_pt","parton highest pt",5,-0.5,4));
  ...
  Softdrop = new SoftDrop(); //XXX
  jwj = new Jetswithoutjets();
  ecf = new ECF();
  varR = new VariableR();
}

void RocHists3::SetIdVersion(TString s)
{
  idVersion=s;
}
...
void RocHists3::Fill()
{
  ...
  double time1=0;
//EFFICIENCY
  if(!IsRealData && (idVersion=="TTbarEff" || idVersion=="TTbarEff_1" || idVersion=="TTbarEff_2" || idVersion=="TTbarEff_3" || idVersion=="Zprime"))
  {
  ...
  std::vector<fastjet::PseudoJet> constituents;
    ...
    /*   if(calc->GetEventNum()==4252236) std::cout<<"cluster"<<inclusive_jetsCA[o].has_valid_cluster_sequence()<<std::endl;
    constituents= SortedSubJets[o].constituents();
     if(calc->GetEventNum()==4252236) std::cout<<"constituents "<<constituents.size()<<std::endl;
     }*/
...
std::vector<fastjet::PseudoJet> fatjet_constituents[fatjets_comb.size()];
 ...
 if(calc->GetEventNum()==4252236){ //jetdisplay 4252236 4251318 4251364
 ...
 //SortedJets = sorted_by_pt(inclusive_jets_varR) ;
  ...
  Hist("NJetstop")->Fill(bcc->topjets->size(),weight);
	...
	if(bestjetindex2>-1 )
	  ...
	  std::vector<fastjet::PseudoJet> SortedSubJets2=subjets[fatjets_comb[bestjetindex2].user_index()-1];
	  ...
	  if(SortedJets[bestjetindex2].m()>150 &&  SortedJets[bestjetindex2].m()<250)  Hist("pT_s1_tobi2_tagged")->Fill(Had_Tops[j].pt(),weight);
    
	  if(SortedSubJets2.size()>1 && SortedJets[bestjetindex2].m()>150 &&  SortedJets[bestjetindex2].m()<250 && mmin2>50)  Hist("pT_s1_tobi3_tagged")->Fill(Had_Tops[j].pt(),weight);
	  if(SortedSubJets2.size()>2 && SortedJets[bestjetindex2].m()>150 &&  SortedJets[bestjetindex2].m()<250 && wmass_rec>70 && wmass_rec<90)  Hist("pT_s1_tobi6_tagged")->Fill(Had_Tops[j].pt(),weight);
	  Hist("pT_jets")->Fill(varjets.at(bestjetindex2).perp(),weight);
	  TString hname1 = TString::Format("pT_gen_match_skt");
	  Hist("pT_gen_match_skt")->Fill(Had_Tops[j].pt(),weight);
	  ...
	  //if(Had_Tops[j].pt()>700) microconesize=0.1;
	    bool CMStag=false;
	   CMStag=CMSTopTagFull_pseudo(jet,3.0,0.05,0.4,0.0004,mjet, nsubjets,mmin,subjets);
	  ...
	  // else CMStag=false;
	  if(CMStag) {
	    double deltaR_tag=sqrt(pow(jet.eta()-Had_Tops[j].eta(),2)+pow(jet.phi_std()-Had_Tops[j].phi(),2));
	    ...
	    Hist("pT_s1_cms_tagged400")->Fill(Had_Tops[j].pt(),weight);
	   	  }
	  ...
	  double distance=100.;
	   ...
	   if(Had_Tops[j].pt()>500&& Had_Tops[j].pt()<600)  Hist("mjet400_1")->Fill(mjet,weight);
	   ...
	   if(Had_Tops[j].pt()>1000 &&Had_Tops[j].pt()<1200)  Hist("mjet1000_1")->Fill(mjet,weight);
	      ...
	      if(nsubjets>2){
	       ...
	       if(Had_Tops[j].pt()>500 &&Had_Tops[j].pt()<600)  Hist("mjet400_2")->Fill(mjet,weight);
	     ...
	     if(Had_Tops[j].pt()>200 && Had_Tops[j].pt()<400) Hist("nsubjets_massjump200")->Fill(nsubjets,weight);
	     if(Had_Tops[j].pt()>400 &&Had_Tops[j].pt()<600) Hist("nsubjets_massjump400")->Fill(nsubjets,weight);
	     if(Had_Tops[j].pt()>600 &&Had_Tops[j].pt()<800) Hist("nsubjets_massjump600")->Fill(nsubjets,weight);
	     if(Had_Tops[j].pt()>800 &&Had_Tops[j].pt()<1000) Hist("nsubjets_massjump800")->Fill(nsubjets,weight);
	     if(Had_Tops[j].pt()>1000 &&Had_Tops[j].pt()<1200) Hist("nsubjets_massjump1000")->Fill(nsubjets,weight);
	   ...
	   fastjet::PseudoJet groomed_jet;//XX
	   ...
	   if(groomed_jet.m()>140 &&  groomed_jet.m()<250)  Hist("pT_s1_tobi4_tagged")->Fill(Had_Tops[j].pt(),weight);
	  ...
	  if(tau3/tau2<roc/100.){
	   ...
	   if(CMStag &&/*nsubjets>2 && mmin>50*/Rmin-Rmin_exp<0.07) for(int roc=0;roc<250;roc++) for(int roc2=0;roc2<roc;roc2++){
		 if(mass_Rmin<roc && mass_Rmin>roc2)  ((TH3D*)Hist("nsub_eff_mass"))->Fill(Had_Tops[j].pt(),roc,roc2,weight);}
	...
	for(unsigned int j=0; j<genparticles->size();j++){
	  if (abs(genparticles->at(j).pdgId())<=6 || abs(genparticles->at(j).pdgId())==21)
	    if(genparticles->at(j).pt()>100) { Hist("pT_s1_all")->Fill(genparticles->at(j).pt(),weight);
	      Hist("pdg_all")->Fill(genparticles->at(j).pdgId(),weight);
	      if( abs(genparticles->at(j).pdgId())==21) Hist("pT_s1_gluons_all")->Fill(genparticles->at(j).pt(),weight);
	      if(abs(genparticles->at(j).pdgId())<=6) Hist("pT_s1_quarks_all")->Fill(genparticles->at(j).pt(),weight);
	      std::cout<<"particle "<<genparticles->at(j).pt()<<" pdg "<<genparticles->at(j).pdgId()<<std::endl;
	    }
	}
  ...
  varjets=SortedSubJets;
   ...
   for(unsigned int i=0; i<varjets.size(); ++i)
   {
	 ...
	 if (DeltaR2<akt_r && genparticles->at(j).pt()>highestPt /*&& fatjets_comb[i].perp()>200*/)
	 {
	   highestpt_particle=j;
     ...
     if(abs(genparticles->at(highestpt_particle).pdgId())<=6) Hist("parton_highest_pt")->Fill(1,weight);
     if(abs(genparticles->at(highestpt_particle).pdgId())==21) Hist("parton_highest_pt")->Fill(3,weight);
     Hist("pdg_highestpt")->Fill(genparticles->at(highestpt_particle).pdgId(),weight);
     //std::cout<<" particle higph pt "<<genparticles->at(highestpt_particle).pt()<<std::endl;
     //   std::cout<<"test2"<<std::endl;
	 ...
	 sum.operator+=(SortedSubJets[mj]);
	  ...
	  if(SortedSubJets.size()>2 && varjets[i].m()>150 &&  varjets[i].m()<250 && wmass_rec>70 &&wmass_rec<90)  Hist("pT_s1_tobi6_tagged")->Fill(genparticles->at(bestjetindex).pt(),weight);
       ...
       //if(genparticles->at(bestjetindex).pt()>700) microconesize=0.1;
         bool CMStag=false;
	 std::cout<<"match "<<genparticles->at(bestjetindex).pt()<<std::endl;
	 if(CMSTopTagFull_pseudo(jet,3.0,0.05,0.4,0.0004,mjet, nsubjets,mmin,subjets)){
	   Hist("pdg_tagged")->Fill(genparticles->at(bestjetindex).pdgId(),weight);
	   if( abs(genparticles->at(bestjetindex).pdgId())==21) Hist("pT_s1_gluons")->Fill(genparticles->at(bestjetindex).pt(),weight);
	   if(abs(genparticles->at(bestjetindex).pdgId())<=6) Hist("pT_s1_quarks")->Fill(genparticles->at(bestjetindex).pt(),weight);



	   }
// std::cout<<"CMS "<<genparticles->at(bestjetindex).pt()<<std::endl;
	 ...
	 double fw=0.15;
       double m12,m13, m23, m123, m_pruned, m_unfiltered;
       //if(HepTopTagFull_pseudo(jet,m12, m13, m23, fw, m123, m_pruned, m_unfiltered))   CMStag=true;
       /*  if(MultiRTopTag_pseudo(jet, calc->GetPFParticles(),fw, Rmin,mass_Rmin,pt_Rmin,mass_diff,pt_for_exp,Rmin_exp,fw_out)&&Rmin-Rmin_exp<0.07 && mass_Rmin>140 && mass_Rmin<250)  
	 {
	   //std::cout<<"OPTIMALR "<<genparticles->at(bestjetindex).pt()<<std::endl;
	   Hist("pdg_tagged")->Fill(genparticles->at(bestjetindex).pdgId(),weight);
	   if( abs(genparticles->at(bestjetindex).pdgId())==21) Hist("pT_s1_gluons")->Fill(genparticles->at(bestjetindex).pt(),weight);
	   if(abs(genparticles->at(bestjetindex).pdgId())<=6) Hist("pT_s1_quarks")->Fill(genparticles->at(bestjetindex).pt(),weight);
	 ...
	 CMStag=true;
       //else CMStag=false;
       if(CMStag){
	 Hist("pT_s1_cms_tagged400")->Fill(genparticles->at(bestjetindex).pt(),weight);
	 ...
	 if(genparticles->at(bestjetindex).pt()>500 &&genparticles->at(bestjetindex).pt()<600)  Hist("mjet400_1")->Fill(mjet,weight);
	 ...
	 if(genparticles->at(bestjetindex).pt()>500 &&genparticles->at(bestjetindex).pt()<600)  Hist("mjet400_2")->Fill(mjet,weight);
	...
	if(/*nsubjets>2 && mmin>50*/Rmin-Rmin_exp<0.07&& CMStag) for(int roc=0;roc<250;roc++) for(int roc2=0;roc2<roc;roc2++){
	      if(mass_Rmin<roc && mass_Rmin>roc2)  ((TH3D*)Hist("nsub_eff_mass"))->Fill(genparticles->at(bestjetindex).pt(),roc,roc2,weight);}
     }
   }}
